...
import pandas as pd
import Anonymize_resource_country_encoding
import os
...
def apply_privacy():
    ...
    for i in range(file_count):
        try:
            privacy_log = pd.read_csv(str(pathlib.Path().resolve()) +
                                      "\\Anonymized Resource Country\\k = " + str(k_value) + "\\anonymized_data_" + str(i) + ".csv", sep=',')
    ...
    xes_exporter.apply(event_log, str(pathlib.Path().resolve()) + '/final_resource_country_log_' + str(k_value) +'_.xes')
