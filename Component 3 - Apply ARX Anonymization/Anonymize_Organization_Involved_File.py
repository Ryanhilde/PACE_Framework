...
import pandas as pd
import Anonymize_organization_involved_encoding
import os
...
k_value = str(20)
final_log = '/final_organization_involved_log_k_' + k_value + '.xes'
    ...
    for i in range(file_count):
        try:
            log_csv = pd.read_csv(str(pathlib.Path().resolve()) +
                                  ...
                                  sep=',')
            encoding = Anonymize_organization_involved_encoding.organization_involved_encoding(log_csv, i)
    ...
    for i in range(file_count):
        try:
            privacy_log = pd.read_csv(str(pathlib.Path().resolve()) +
                                      "\\Anonymized Organization Involved\\k = " + k_value + "\\anonymized_data_" + str(
                i) + ".csv",
                                      sep=',')
