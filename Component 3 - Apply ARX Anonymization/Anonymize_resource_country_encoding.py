...
class resource_country_encoding:
    ...
    def write_variant(self):
        ...
        for i in range(self.trace_length):
            country_resource = 'resource country' + str(i)
            attribute_header.append(country_resource)

        for i in event_log:
            attribute_value = []
            for j in range(self.trace_length):
                attribute_value.append(i[j]['resource country'])
    ...
    def apply_privacy(self, privacy_log_csv, event_log):
        ...
        for i in range(self.trace_length):
            country_resource.append(privacy_log_csv['resource country' + str(i)])

        country_resource_df = pd.concat(country_resource).reset_index()
        count = 0
        for i in event_log:
            for j in range(self.trace_length):
                i[j]['resource country'] = country_resource_df[0][count]
                count += 1
