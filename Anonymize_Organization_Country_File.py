...
import pandas as pd
import Anonymize_organization_country_encoding
import os
...
k_value = str(20)
final_log = '/final_organization_country_log_k_' + k_value + '.xes'
    ...
    for i in range(file_count):
        try:
            log_csv = pd.read_csv(str(pathlib.Path().resolve()) +
                                  ...
                                  sep=',')
            encoding = Anonymize_organization_country_encoding.organization_country_encoding(log_csv, i)
    ...
    for i in range(file_count):
        try:
            privacy_log = pd.read_csv(str(pathlib.Path().resolve()) +
                                      "\\Anonymized Organization Country\\k = " + k_value + "\\anonymized_data_" + str(
                i) + ".csv",
                                      sep=',')
            ...
            #csv_file_df.drop(
            #    ['org:group', 'org:resource', 'resource country', 'organization involved', 'org:role', 'impact',
            #     'product'],
            #    inplace=True, axis=1)
